...
function MiniPoster({ post, href }: { post: SitePost; href: string }) {
  return (
    <Link href={href} className={`group block w-[230px] shrink-0 ${dc.motion.fade}`}>
      <article className="relative overflow-hidden rounded-[1.65rem] border border-black/[0.07] bg-[var(--slot4-surface-bg)] p-2 shadow-[0_12px_40px_rgba(0,0,0,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_70px_rgba(0,0,0,0.14)]">
        <div className="relative aspect-[4/5] overflow-hidden rounded-[1.25rem] bg-[var(--slot4-media-bg)]">
          ...
          <span className="absolute left-3 top-3 rounded-full bg-white/92 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-page-text)] shadow-sm">
            Read
          </span>
...
function FeatureTile({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  if (style === 0) {
    return (
      <Link href={href} className="group relative min-h-[360px] overflow-hidden rounded-[2rem] bg-[var(--slot4-dark-bg)] p-5 text-white shadow-[0_18px_70px_rgba(0,0,0,0.14)] transition duration-300 hover:-translate-y-1">
  ...
  if (style === 1) {
    return (
      <Link href={href} className={`group grid overflow-hidden rounded-[2rem] border ${pal.border} bg-[var(--slot4-surface-bg)] shadow-[0_12px_40px_rgba(0,0,0,0.08)] transition duration-300 hover:-translate-y-1 md:grid-cols-[0.82fr_1fr]`}>
  ...
  return (
    <Link href={href} className={`group relative overflow-hidden rounded-[2rem] border ${pal.border} bg-[var(--slot4-accent-soft)] p-6 shadow-[0_12px_40px_rgba(0,0,0,0.08)] transition duration-300 hover:-translate-y-1`}>
...
function WideStoryCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group grid gap-4 overflow-hidden rounded-[1.75rem] border ${pal.border} bg-[var(--slot4-surface-bg)] p-3 shadow-[0_12px_40px_rgba(0,0,0,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_70px_rgba(0,0,0,0.14)] sm:grid-cols-[150px_minmax(0,1fr)]`}>
      <div className="relative aspect-[5/4] overflow-hidden rounded-[1.25rem] bg-[var(--slot4-media-bg)] sm:aspect-square">
        <img src={getEditablePostImage(post)} alt={post.title} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" />
        <span className="absolute bottom-3 left-3 rounded-full bg-black/72 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-white backdrop-blur">
          Pick {index + 1}
        </span>
      </div>
...
function IndexPill({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group relative overflow-hidden rounded-[1.55rem] border ${pal.border} bg-[var(--slot4-surface-bg)] p-5 shadow-[0_12px_40px_rgba(0,0,0,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_70px_rgba(0,0,0,0.14)]`}>
...
export function EditableHomeHero({ primaryTask, primaryRoute, posts }: HomeSectionProps) {
  ...
  return (
    <section className="relative overflow-hidden border-b border-black/[0.08] bg-[var(--slot4-page-bg)]">
      ...
      <div className="relative mx-auto grid max-w-[1180px] gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:gap-12 lg:px-8 lg:py-18">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-black/[0.08] bg-white/80 px-4 py-2 text-xs font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-accent)] shadow-sm">
          ...
          <div className="mt-10 grid max-w-2xl overflow-hidden rounded-[1.15rem] border border-black/[0.08] bg-white/82 shadow-[0_12px_40px_rgba(0,0,0,0.08)] sm:grid-cols-3">
            {[
              ...
              <div key={label} className="border-b border-black/[0.07] px-4 py-4 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0">
                <p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-black/42">{label}</p>
        ...
        <div className="relative">
          <div className="rounded-[1.45rem] border border-black/[0.08] bg-[var(--slot4-dark-bg)] p-3 shadow-[0_18px_70px_rgba(0,0,0,0.14)]">
            <div className="grid gap-3 lg:grid-cols-[1fr_0.62fr]">
              <Link href={leadPost ? postHref(primaryTask, leadPost, primaryRoute) : primaryRoute} className="group relative min-h-[430px] overflow-hidden rounded-[1.15rem] bg-black text-white">
                ...
                <div className="relative z-10 flex min-h-[430px] flex-col justify-between p-6 sm:p-7">
                  <div className="flex items-center justify-between gap-3">
                    <span className="rounded-full bg-[var(--slot4-surface-bg)] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-black">Lead article</span>
                    <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-white/62">{slot4BrandConfig.siteName}</span>
                  </div>
                  <div>
                    <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-white/62">Today's cover</p>
                    <h2 className="mt-3 line-clamp-3 max-w-xl text-3xl font-extrabold leading-[0.98] tracking-[-0.06em] sm:text-4xl">{leadPost?.title || pagesContent.home.hero.featureCardTitle}</h2>
              ...
              <div className="grid gap-3">
                <div className="rounded-[1.15rem] border border-white/10 bg-white/[0.07] p-5 text-white">
                  ...
                  <p className="mt-4 text-xs font-extrabold uppercase tracking-[0.2em] text-white/45">Editorial promise</p>
                  <p className="mt-2 text-xl font-extrabold leading-tight tracking-[-0.04em]">Premium reading paths for curious, focused readers.</p>
                </div>
                {supportingPosts.map((post, index) => (
                  <Link key={post.id || post.slug} href={postHref(primaryTask, post, primaryRoute)} className="group grid grid-cols-[78px_minmax(0,1fr)] gap-3 rounded-[1.1rem] border border-white/10 bg-white/[0.06] p-3 text-white transition hover:bg-white/[0.1]">
                    ...
                    <div className="min-w-0">
                      <p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-white/42">Pick {index + 1}</p>
                      <h3 className="mt-2 line-clamp-3 text-sm font-extrabold leading-tight tracking-[-0.03em]">{post.title}</h3>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="mx-auto -mt-5 grid max-w-[92%] grid-cols-3 overflow-hidden rounded-[1rem] border border-black/[0.08] bg-[var(--slot4-surface-bg)] shadow-[0_12px_40px_rgba(0,0,0,0.08)]">
            {['Brief', 'Depth', 'Signal'].map((item) => (
              <div key={item} className="border-r border-black/[0.08] px-4 py-3 text-center last:border-r-0">
                <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-black/42">{item}</p>
...
export function EditableStoryRail({ primaryTask, primaryRoute, posts }: HomeSectionProps) {
  ...
  return (
    <section className="relative border-t border-black/[0.06] bg-[var(--slot4-surface-bg)]">
      <div className="relative mx-auto max-w-[1180px] px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href={primaryRoute} className="hidden items-center gap-2 rounded-full border border-black/[0.08] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold shadow-sm sm:inline-flex">See all <ArrowRight className="h-4 w-4" /></Link>
        </div>
        <Rail className="mt-8">
...
export function EditableMagazineSplit({ primaryTask, primaryRoute, posts }: HomeSectionProps) {
  ...
  return (
    <section className="relative overflow-hidden border-y border-black/[0.06] bg-[var(--slot4-cream)]">
...
export function EditableTimeCollections({ primaryTask, primaryRoute, posts, timeSections }: HomeSectionProps) {
  ...
  return (
    <section className="bg-[var(--slot4-page-bg)]">
      <div className="mx-auto grid max-w-[1180px] gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:px-8">
        <div>
          ...
          <form action="/search" className="mt-8 flex max-w-md rounded-full border border-black/[0.08] bg-[var(--slot4-surface-bg)] p-2 shadow-sm">
      ...
      {feature ? (
        <div className="mx-auto grid max-w-[1180px] gap-8 px-4 pb-16 sm:px-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1fr)] lg:px-8">
          <Link href={postHref(primaryTask, feature, primaryRoute)} className="group relative min-h-[420px] overflow-hidden rounded-[2rem] bg-black text-white shadow-[0_18px_70px_rgba(0,0,0,0.14)]">
