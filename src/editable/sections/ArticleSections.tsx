  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-12 sm:pt-16 lg:pt-20`}>
        <div className={`rounded-[2.5rem] border ${pal.border} ${pal.darkBg} p-7 text-white shadow-[0_18px_70px_rgba(0,0,0,0.14)] sm:p-10 lg:p-14`}>
          ...
          <form action={basePath} className="mt-8 flex max-w-xl flex-col gap-3 sm:flex-row">
            ...
            <button className={`rounded-full ${pal.accentSoftBg} px-6 py-3 text-sm font-extrabold ${pal.panelText}`}>Filter</button>
          </form>
        </div>
      </section>
      ...
      <section className={`${dc.shell.section} ${dc.shell.sectionY}`}>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
          {pagination.hasPrevPage ? <Link href={pageHref(page - 1)} className={`rounded-full border ${pal.border} bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold`}>Previous</Link> : null}
          ...
          {pagination.hasNextPage ? <Link href={pageHref(page + 1)} className={`rounded-full border ${pal.border} bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold`}>Next</Link> : null}
        </div>
      </section>
    </main>
  )
  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-10 sm:pt-14 lg:pt-16`}>
        <div className={`grid gap-6 rounded-[2.5rem] border ${pal.border} bg-[var(--slot4-surface-bg)] p-6 shadow-[0_18px_70px_rgba(0,0,0,0.14)] lg:grid-cols-[minmax(0,1fr)_320px] lg:p-10`}>
          <div className="min-w-0">
            ...
            <h1 className={`mt-4 max-w-4xl text-4xl font-extrabold leading-[0.98] tracking-[-0.07em] ${pal.panelText} sm:text-5xl lg:text-7xl`}>{post?.title || pagesContent.detailPages.article.fallbackTitle}</h1>
          </div>
          <aside className={`min-w-0 rounded-[2rem] ${pal.darkBg} p-6 text-white`}>
            ...
            <Link href="/contact" className={`mt-6 inline-flex items-center gap-2 rounded-full bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold ${pal.panelText}`}>Contact <ArrowRight className="h-4 w-4" /></Link>
          </aside>
        </div>
      </section>
      <section className="mx-auto w-full max-w-5xl px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pb-24">
        <div className={`rounded-[2.25rem] border ${pal.border} bg-[var(--slot4-surface-bg)] p-6 shadow-[0_18px_70px_rgba(0,0,0,0.14)] sm:p-8 lg:p-10`}>
