...
const getSummary = (post: SitePost) => toPlainText(post.summary) || toPlainText(getContent(post).description) || toPlainText(getContent(post).excerpt) || toPlainText(getContent(post).body)
...
const taskDeck: Record<TaskKey, { icon: typeof FileText; archiveClass: string; promise: string; badge: string }> = {
  article: { icon: FileText, archiveClass: 'grid gap-5 md:grid-cols-2 lg:grid-cols-3', promise: 'Readable article cards with room for headlines, excerpts, images, and categories.', badge: 'Read' },
    ...
    <EditableSiteShell>
      <main className="bg-[var(--slot4-page-bg)] text-[var(--slot4-dark-bg)]">
        <section className="mx-auto grid max-w-[1180px] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-16">
          <div className="rounded-[1.35rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-7 shadow-[0_18px_70px_rgba(0,0,0,0.14)] sm:p-10">
            <div className="inline-flex items-center gap-2 rounded-full border border-black/[0.06] bg-white/70 px-4 py-2 text-xs font-extrabold uppercase tracking-[0.24em] text-[var(--slot4-accent)]"><Icon className="h-4 w-4" /> {label}</div>
            <h1 className="mt-5 max-w-4xl text-5xl font-extrabold leading-[0.95] tracking-[-0.07em] sm:text-6xl">{voice?.headline || `Browse ${label}`}</h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={basePath} className="rounded-full bg-[var(--slot4-dark-bg)] px-5 py-3 text-sm font-extrabold text-[var(--slot4-page-bg)]">Browse all</Link>
              <Link href="/search" className="rounded-full border border-black/[0.06] px-5 py-3 text-sm font-extrabold">Search posts</Link>
            </div>
          </div>

          <form action={basePath} className="self-end rounded-[1.25rem] border border-black/[0.06] bg-white/75 p-5 shadow-sm backdrop-blur">
            <div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.2em] opacity-55"><Filter className="h-4 w-4" /> Filter</div>
            <select name="category" defaultValue={category} className="mt-4 h-12 w-full rounded-2xl border border-black/[0.06] bg-[var(--slot4-surface-bg)] px-4 text-sm font-bold outline-none">
              <option value="all">All categories</option>
              {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
            </select>
            <button className="mt-3 h-12 w-full rounded-2xl bg-[var(--slot4-dark-bg)] text-sm font-extrabold text-[var(--slot4-page-bg)]">Apply</button>
        ...
        <section className="mx-auto max-w-[1180px] px-4 pb-16 sm:px-6 lg:px-8">
          {posts.length ? (
            ...
            <div className="rounded-[1.25rem] border border-dashed border-black/[0.06] bg-white/60 p-10 text-center">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border border-black/[0.06] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold">Previous</Link> : null}
            <span className="rounded-full bg-[var(--slot4-dark-bg)] px-5 py-3 text-sm font-extrabold text-[var(--slot4-page-bg)]">Page {page} of {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border border-black/[0.06] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-extrabold">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
...
function ArticleArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className="group overflow-hidden rounded-[1.25rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group grid gap-5 rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl sm:grid-cols-[120px_1fr]">
      <div className="flex h-28 w-28 items-center justify-center overflow-hidden rounded-[1.5rem] bg-[var(--slot4-page-bg)] ring-1 ring-black/[0.06]">
        {logo ? <img src={logo} alt="" className="h-full w-full object-cover" /> : <BriefcaseBusiness className="h-10 w-10 opacity-45" />}
      </div>
      <div className="min-w-0">
        <div className="flex flex-wrap gap-2">
          <span className="rounded-full bg-[var(--slot4-dark-bg)] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-page-bg)]">Directory</span>
          {location ? <span className="inline-flex items-center gap-1 rounded-full border border-black/[0.06] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.14em]"><MapPin className="h-3 w-3" /> {location}</span> : null}
        </div>
...
function ClassifiedArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group overflow-hidden rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      <div className={index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}>
        <img src={image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
      </div>
      <div className="p-5">
        <div className="inline-flex items-center gap-2 rounded-full bg-[var(--slot4-page-bg)] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.16em]"><ImageIcon className="h-3 w-3" /> Visual</div>
        <h2 className="mt-4 line-clamp-3 text-xl font-extrabold leading-tight tracking-[-0.04em]">{post.title}</h2>
      </div>
    </Link>
  )
}
...
function BookmarkArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className="group block rounded-[1.7rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-6 shadow-sm transition hover:-translate-y-1 hover:bg-[var(--slot4-dark-bg)] hover:text-[var(--slot4-page-bg)]">
...
function PdfArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-6 text-center shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
